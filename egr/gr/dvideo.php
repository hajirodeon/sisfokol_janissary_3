...
if ($s == "edit")
	...
	//query
	$qnil = mysqli_query($koneksi, "SELECT * FROM guru_mapel_video ".
...
if ($_POST['btnHPS'])
	...
	do
		...
		//del data
		mysqli_query($koneksi, "DELETE FROM guru_mapel_video ".
						"WHERE kd_guru_mapel = '$gmkd' ".
						"AND kd = '$kd'");
		}
...
if ($_POST['btnSMP'])
	...
	if ((empty($judul)) OR (empty($url)))
		...
		if ($s == "edit")
			{
			//update
			mysqli_query($koneksi, "UPDATE guru_mapel_video SET judul = '$judul', ".
							"url = '$url' ".
							"WHERE kd_guru_mapel = '$gmkd' ".
							"AND kd = '$linkkd'");
		...
		if (empty($s))
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM guru_mapel_video ".
									"WHERE kd_guru_mapel = '$gmkd' ".
									"AND judul = '$judul'");
			...
			if ($tcc != 0)
				...
				{
				//insert data
				mysqli_query($koneksi, "INSERT INTO guru_mapel_video(kd, kd_guru_mapel, judul, url, postdate) VALUES ".
