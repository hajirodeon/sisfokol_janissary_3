...
if (($s == "hapus") AND (!empty($artkd)))
	...
	//del data
	mysqli_query($koneksi, "DELETE FROM guru_mapel_news ".
					"WHERE kd = '$artkd'");


	//query
	$qcc = mysqli_query($koneksi, "SELECT * FROM guru_mapel_news_filebox ".
	...
	//hapus query
	mysqli_query($koneksi, "DELETE FROM guru_mapel_news_filebox ".
					"WHERE kd_guru_mapel_news = '$artkd'");
...
if ($_POST['btnSMPx'])
	...
	if ((empty($kategorix)) OR (empty($judulx)) OR (empty($rangkumanx)) OR (empty($isix)))
		...
		if ($s == "baru")
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT guru_mapel_news.*, guru_mapel_news.kd AS nkd ".
									"FROM guru_mapel_news ".
									"WHERE guru_mapel_news.kd_guru_mapel = '$gmkd' ".
									"AND guru_mapel_news.kategori = '$kategorix' ".
			...
			if ($tcc != 0)
				...
				{
				//insert
				mysqli_query($koneksi, "INSERT INTO guru_mapel_news(kd, kd_guru_mapel, kategori, judul, rangkuman, isi, postdate) VALUES ".
		...
		if ($s == "edit")
			{
			//update
			mysqli_query($koneksi, "UPDATE guru_mapel_news SET kategori = '$kategorix', ".
							"judul = '$judulx', ".
	    ...
	    <div class="col-lg-12">
	      ...
	      <div class="box box-warning">
	        ...
	        <div class="box-body">
				...
				<a href="'.$filenya.'&s=baru&artkd='.$x.'" title="Entry Baru" class="btn btn-danger">Entry Baru >></a>
				<hr>';
			...
			if (($s == "edit") OR ($s == "baru"))
				...
				//nilai
				//query
				$qdt = mysqli_query($koneksi, "SELECT guru_mapel_news.*, guru_mapel_news.kd AS nkd ".
										"FROM guru_mapel_news ".
										"WHERE guru_mapel_news.kd_guru_mapel = '$gmkd' ".
				...
				do
					...
					echo '<option value="'.$kat_kat.'">'.$kat_kat.'</option>';
					}
				...
				<p>
				<strong>Rangkuman (maksimal tiga kalimat): </strong>
				<br>
			...
			//jika daftar //////////////////////////////////////////////////////////////////////////////////
				...
				if ($count != 0)
					...
					do
						...
						if (($s == "view") AND ($artkd == $dt_kd))
							...
							if ($a == "hapus")
								{
								//hapus
								mysqli_query($koneksi, "DELETE FROM guru_mapel_news_msg ".
												"WHERE kd_guru_mapel_news = '$artkd' ".
												"AND kd = '$msgkd'");
						...
						//jika beri komentar
							...
							if ($_POST['btnSMP'])
								...
								//query
								mysqli_query($koneksi, "INSERT INTO guru_mapel_news_msg(kd, kd_guru_mapel_news, dari, msg, postdate) VALUES ".
							...
							//view
							echo '<br>
							<textarea name="bk_news" rows="5" wrap="virtual" class="btn-block"></textarea>
							<br>
							<input name="artkd" type="hidden" value="'.$artkd.'">
					...
					{
					echo '<p>
					<font color="blue"><strong>TIDAK ADA BERITA. Silahkan Entry Dahulu...!!</strong></font>
					</p>';
					}
...
$judulx = "[$pel_tapel. $pel_kelas]. $pel_nm [BERITA].";
